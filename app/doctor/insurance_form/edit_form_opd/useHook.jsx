...
export default function useHook({
  ...
  const { EditOrderInsurancePartA } = useApiRequest();
  ...
  const handleSubmit = async (value) => {
    ...
    try {
      const data = await EditOrderInsurancePartA(value, hosClaimId);
      // setPatData(null);
      onClose();
      form.reset();
      if (data) {
  ...
  useEffect(() => {
    ...
    form.setFieldValue("claimId", claimData.id || "");
  ...
  const iso = claimData?.hospitalForm?.datetimeForm;
  ...
  useEffect(() => {
    ...
    setDate({
      datetimeForm: parseDate(bangkok.toISOString().split("T")[0]),
    });

    // ⚠️ อย่าส่ง JS Date เข้า DatePicker
    form.setFieldValue("datetimeForm", bangkok);
  }, [iso]);
